...
class CustomLiveReloadServerTask extends Task {
  ...
  static getPath(options) {
    return `//${options.host || options.liveReloadHost}:${
  ...
  start(startOptions) {
    ...
    return this.liveReloadServer.listen(
      startOptions.liveReloadPort,
      startOptions.liveReloadHost
    );
  }
  ...
  shouldTriggerReload(options) {
    ...
    if (this.project.liveReloadFilterPatterns.length > 0) {
      ...
      result = this.project.liveReloadFilterPatterns.every(
        (pattern) => pattern.test(filePath) === false
      );
  ...
  didChange(results) {
    ...
    if (results.stack) {
      ...
      this.tree = new FSTree.fromEntries(
        this.getDirectoryEntries(results.directory),
        { sortAndExpand: true }
      );
      files = previousTree
